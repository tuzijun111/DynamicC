...
//    }
    ...
    public static ArrayList<ArrayList<Integer>> Dy_DBSCAN (ArrayList<ArrayList<Integer>> cluster, ArrayList<Integer> ttt, ArrayList<Hashtable> adj) throws IOException, ClassNotFoundException {
        // preprocess the adjacent matrix adj to make sure that it only contains points that will appear currently, this step is necessary for remove and update operations
        for (int i=1;i<adj.size();i++){
            ttt.add(i); //one by one increment i.e. add operation
            ArrayList<Hashtable> current_adj = (ArrayList<Hashtable>) deepCopy1(adj);
//            ArrayList<Hashtable> current_adj = new ArrayList<>();
//            precluster = (ArrayList<ArrayList<Integer>>) deepCopy(inter);
            if( (i%50==0)){
                for (int j =0; j<ttt.size(); j++) {
                    Enumeration enu = current_adj.get(ttt.get(j)).keys();

                    while (enu.hasMoreElements()) {
                        int p = (int) enu.nextElement();
                        if ( p>50) //delete the points which is larger than current largest point from adj
                            current_adj.get(ttt.get(j)).remove(p);
                    }
                }
                Cluster.dynamic_DBSCAN(cluster, ttt, current_adj);
            }
        }
        return cluster;
    }
...
//        }
    ...
    public static ArrayList<Hashtable> deepCopy1(ArrayList<Hashtable> src) throws IOException, ClassNotFoundException {
        ByteArrayOutputStream byteOut = new ByteArrayOutputStream();
        ObjectOutputStream out = new ObjectOutputStream(byteOut);
        out.writeObject(src);
        ...
        ObjectInputStream in = new ObjectInputStream(byteIn);
        @SuppressWarnings("unchecked")
        ArrayList<Hashtable> dest = (ArrayList<Hashtable>) in.readObject();
        return dest;
    }
