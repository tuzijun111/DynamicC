...
//        }
    ...
    public static double[][] EuclidAdj_DB(ArrayList<ArrayList<String>> abc, int k[], int minDis) throws IOException {  //k[] is used to specify the attributes
...
//                }
    ...
    public static double k_means_score(ArrayList <ArrayList<Integer>> result, ArrayList<ArrayList<String>> abc) {
        //note that for the DBSCAN clustering results, we need to ignore the 0-row since it store the outliers
        int k[]={1};   //specify the attributes
        double score = 0;
        for (int i = 1; i < result.size(); i++) {  //ignore the 0-row since it stores the outliers
        //for (int i = 0; i < result.size(); i++) {
            double temp_score = 0;
            double mean[] = {0, 0};
            for (int j = 0; j < result.get(i).size(); j++) {
                for (int p = 0; p < k.length; p++) {
                    mean[p] += Integer.parseInt(abc.get(result.get(i).get(j)).get(k[p]));  // get its attribute value and compute the sum
                }
            }
            for (int p = 0; p < k.length; p++) {
                mean[p] = mean[p]/result.get(i).size();   //compute the mean point
            }
            for (int q = 0; q < result.get(i).size(); q++) {
                for (int p = 0; p< k.length; p++) {
                    double temp = Math.pow(Integer.parseInt(abc.get(result.get(i).get(q)).get(k[p])) - mean[p], 2);
                    temp_score += temp;
                }
            }
            score += Math.sqrt(temp_score);
        }
    ...
    public static ArrayList<ArrayList<String>> CosineNgram() throws IOException { //n=3 for trigram
        ...
        abc = DataPro.CsvData("/Users/binbingu/Documents/Datasets/Music Brzinz/20K.csv", 1);
