...
//    }
    ...
    public static List<Pair> Goldpair(String filepath) throws IOException{
        ...
        data = DataPro.CsvData(filepath, 1);
...
//translate the groundtruth into pairs
    public static void IntoPairs() throws IOException{
        ArrayList<ArrayList<String>> data = new ArrayList<ArrayList<String>>();
        ArrayList<ArrayList<Integer>> cluster = new ArrayList<ArrayList<Integer>>();
        data = DataPro.CsvData("/Users/binbingu/Documents/Datasets/Music Brzinz/20K.csv",1);
        ArrayList<Integer> item = new ArrayList<Integer>();
...
//        }
    ...
    public static void InPairForAnyNum() throws IOException{
        ArrayList<ArrayList<String>> data = new ArrayList<ArrayList<String>>();
        ArrayList<ArrayList<Integer>> cluster = new ArrayList<ArrayList<Integer>>();
        data = DataPro.CsvData("/Users/binbingu/Documents/Datasets/Music Brzinz/goldpair.csv", 1);
...
//transform the arraylist into pairs
    ...
    public static double F1_measure(ArrayList<ArrayList<Integer>> Gold, ArrayList<ArrayList<Integer>> Prediction){   //two clustering results as input
        List<Pair> list1 = new ArrayList<Pair>();
        List<Pair> list2 = new ArrayList<Pair>();
        ArrayList<ArrayList<Integer>> ab = Evaluate.ClusterTo2D(Gold);
        ArrayList<ArrayList<Integer>> cd = Evaluate.ClusterTo2D(Prediction);
        for (int i=0;i<ab.size();i++) {
        ...
        for (int j=0;j<Prediction.size();j++) {
            Pair<Integer, Integer> B =  new Pair <Integer, Integer> (cd.get(j).get(0), cd.get(j).get(1));
            list2.add(B);
        }
